    ...
    public Describe getDescribeByDesId(int DesId) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                for (int i = 0; i < res1_kq.length; i++) {
                    res1_kq[i] = res1_kq[i].trim();
                }
                d.setThongTinSanPham(res1_kq);
                String res2 = rs.getString("DacDiemNoiBat");
                String[] res2_kq = res2.split(".n@");
                for (int i = 0; i < res2_kq.length; i++) {
                    res2_kq[i] = res2_kq[i].trim();
                }
    ...
    public void insertProduct(Product p, Describe d) {
        try {
            ...
            if (rs1.next()) {
                ...
                String sql2 = "insert Describes(DesId, MatKinhCamUng, DoPhanGiai, ManHinhRong, CameraSau, QuayPhim,"
                        + "HeDieuHanh, Chipset, Ram, Rom, Pin, ThongTinSanPham, DacDiemNoiBat)"
                        + "values (?,?,?,?,?,?,?,?,?,?,?,?,?)";
